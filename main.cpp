...
int main() {
	...
	for (int i = 0; i < PAGE_STREAM_SIZE; i++) {
		...
		for (int j = 0; j < numPages; j++) {
			newRow->row[j] = pageFramesBelady->back()->row[j];
		}
	...
	for (int i = 0; i < PAGE_STREAM_SIZE; i++) {
		...
		std::shared_ptr<Row> newRow = std::shared_ptr<Row>(new Row(numPages));
		for (int j = 0; j < numPages; j++) {
			newRow->row[j] = pageFramesFIFO->back()->row[j];
		}
	...
	for (int i = 0; i < PAGE_STREAM_SIZE; i++) {
		...
		std::shared_ptr<Row> newRow = std::shared_ptr<Row>(new Row(numPages));
		std::shared_ptr<Row> topRow = pageFramesLFU->back();
		for (int j = 0; j < numPages; j++) {
			newRow->row[j] = pageFramesLFU->back()->row[j];
		}
		pageFramesLFU->push_back(newRow);
